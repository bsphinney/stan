...
## Evosep column changes vs backpressure (learned 2026-09-09)

Scored the pressure-based column-change detector against Gabriela's
operator record of nine timsTOF HT changes (2025-07-07 → 2026-09-04),
all confirmed with her directly. Five moved the baseline, four did not.

| Problem | Solution |
|---------|----------|
| A logged column change with **no** pressure step reads as a bad log entry | It is often a real change and a *diagnosis*: the restriction is not in the column. Three columns went in between 2026-05-15 and 07-30 while resistance climbed 185 → 219 bar/(µL·min⁻¹) uninterrupted; the ramp only cleared on 07-30 when a new **tip** went in alongside the column (run 23229 `…oldtip-new-zdf-column…`, 23232 first with the new tip). Look upstream — emitter, capillary, tip — before ordering another column. |
| `detect_steps` misses real changes | `STEP_FRAC` is 6 %. The 2026-03-12 change is a sustained −4.6 %, plainly visible in the 100 **and** 60 SPD series and held for weeks. A replacement column need not be much less restrictive than the one it replaces, so `column_lifetimes` is a lower bound by however many changes sit under the floor. |
| `detect_steps` also **invents** changes | Worse than the misses, and it was the larger error. A four-run hold window let any dip lasting a few injections pass as an intervention. Measured over the whole 27,310-run record, 7 of 20 boundaries were excursions the level came straight back from — 2025-07-02 dipped 200 → 135 for four injections, returned to 200, and carved a 27-injection column five days before the operator's real 07-07 change. Fixed in v1.1.1: the new level must hold across ≥12 runs **and** ≥48 h, capped at 7 days. Validate any change to this against the `held?` test — does the 7-day median sit nearer the new level or the old one? |
| A column change that clears nothing | `column_lifetimes` emits `install_cleared_nothing` and `changes_that_cleared_nothing` (v1.1.1) when a swap fails to lower resistance **and** the outgoing level was elevated over the healthy baseline. Both conditions matter: a routine swap on a healthy system also fails to drop anything, and flagging that teaches people to ignore the flag. Known marginal case documented at `COLUMN_HEALTHY_LOOKBACK_D` — slow secular drift can read as restriction. |
| Reading steps off `daily.plateau_median_bar` | Don't — it mixes methods, so it oscillates ~190/~290 as 100 SPD or 60 SPD dominates a given day and looks like constant stepping. Use flow-normalised resistance (`plateau_bar / plateau_flow_ul_min`) for **one** method, which is what the detector does. |
| Confirming a change against a single method | Cross-check the second method. Every real boundary above appears in both 100 and 60 SPD within the hour; the sub-threshold 2026-03-12 change is believable precisely because both series drop together. |
| An `sbatch --wrap` holding a nested-quoted Python guard | Don't. Two layers of ssh + wrap quoting silently stripped the inner quotes, the guard raised `NameError: name 'summary' is not defined`, its `|| rm` branch fired and deleted a good 856 KB document after a 16-minute extract. `ev_full_publish.sh` exists for this reason — put it in a file. |
