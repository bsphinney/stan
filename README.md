...
- [ ] cIRT retention-time trend plot on local dashboard: for a curated panel of ~10 endogenous HeLa peptides that elute across the gradient, chart observed RT over time with one line per peptide. CRITICAL: split by SPD — same peptide elutes at very different absolute times on 30-SPD vs 100-SPD (toggle/selector or separate panels per SPD). Purpose: detect LC column drift, gradient shape changes, and solvent problems that don't show in ID counts. Panel must be instrument-family specific; tested against /Volumes/proteomics-grp/STAN/TIMS-10878/baseline_output at SPD=100 and found 2,473 endogenous peptides with RT_CV <5% across 25 good runs, so there's no shortage of stable anchors. Build the panel empirically per instrument_family: take runs passing a quality gate (>15k precursors for DIA), find peptides present in ≥90% of runs with RT_CV <5%, bin by RT and pick 10 spread anchors. Persist anchor list in a new `irt_anchors` table keyed by (instrument_family, spd, peptide) so it's stable across sessions. **Separate but related**: `stan/metrics/chromatography.py:269 DEFAULT_IRT_LIBRARY` is the Biognosys 11-peptide spike-in set (LGGNEQVTR et al.) — UC Davis HeLa QC has zero of those peptides at 1% FDR across the reports I checked, so `irt_max_deviation_min` and `n_irt_found` are computing 0/0 on every run and the metric is effectively dead. When implementing the cIRT plot, replace the hardcoded Biognosys set with the empirically-built anchor panel (or mark Biognosys peptides as optional and only compute that metric when any spike-in peptide is actually seen)
