...
def import_archive(archive_path: Path, skip_duplicates: bool = True) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with tarfile.open(archive_path, "r:gz") as tar:
            # Use the "data" filter on Python 3.12+ to silence the deprecation
            # warning and prepare for Python 3.14 where a filter is required.
            extract_kwargs: dict = {}
            if hasattr(tarfile, "data_filter"):
                extract_kwargs["filter"] = "data"
            tar.extractall(tmpdir, **extract_kwargs)
